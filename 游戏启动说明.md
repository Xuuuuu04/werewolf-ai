...
项目使用环境变量加载 API 配置，密钥不再硬编码在仓库中：

- **API端点**: 环境变量 `LLM_BASE_URL`
- **API密钥**: 环境变量 `LLM_API_KEY`
- **模型名称**: 环境变量 `LLM_MODEL`（默认 `qwen3-coder-plus`）
...
如需修改模型配置，请编辑对应的 `.example.yaml` 模板，再复制为 `.yaml` 后使用。
