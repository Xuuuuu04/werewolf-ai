...
- 获胜条件（采用标准9人局「屠边规则」）：当游戏中有一个群体实现它们的获胜条件时游戏结束。如果所有狼人被淘汰，村民就获胜；如果狼人杀光了所有普通村民 **或** 杀光了所有神职（预言家/女巫/守卫/猎人），狼人就获胜。
- 狼刀在先：当白天公布出局讯息后，若同时满足双方胜利条件，狼人获胜。
...
- 倒钩：狼人在发言中针对同伙狼人，制造对立假象
- 站边：根据发言和逻辑判断，选择相信哪一方的预言家
- 归票：发言结束后，统一投票方向，推出狼人
- 抿身份：通过发言内容和行为，推测某个玩家的真实身份
- 自刀：狼人在夜晚杀害自己的队友，制造银水身份
- 推人：通过发言引导大家投票某个玩家
- 闷枪：猎人被女巫毒杀后无法开枪带人
...
- 1位预言家：
    - 目标：预言家的目的是帮助村民识别狼人。
    - 能力：在夜晚阶段，预言家可以秘密选择一名玩家，每晚了解他的真实身份（是否为狼人）。预言家获得的信息称为"金水"（好人）或"查杀"（狼人）。
- 1位女巫：
    - 目标：女巫的目的是策略性地使用她的特殊能力来帮助村民。
    - 能力：女巫有一瓶解药和一瓶毒药。一旦使用，后续回合中不能再用。女巫不能在同一晚既使用解药又使用毒药。解药可以用来救一名在夜间被狼人猎杀的玩家（被救玩家称为"银水"）。毒药可以淘汰一名很可能是狼人的玩家。
- 1位守卫：
    - 目标：守卫的目的是策略性地使用他的特殊能力来帮助村民。
    - 能力：守卫每晚可以保护一名玩家，防止他们受到狼人的攻击。守卫可以选择保护自己，或者选择不保护任何人，但他不能在连续两个夜晚保护同一个玩家。注意：若守卫守护的目标同时被女巫解救，则两种效果互相抵消，玩家依然死亡（俗称"奶穿"）。
其他的都是普通村民。
...
- 昼夜更替：游戏有交替的白天和黑夜阶段。夜里，狼人秘密选择一名村民猎杀。白天，所有玩家讨论并投票决定他们认为是狼人的玩家，票数最多的玩家被淘汰。
- 特殊角色：游戏中有存在一些有特殊能力的角色，比如能得知玩家身份的“预言家”、具有治疗和毒杀能力的“女巫”等。
- 获胜条件（标准9人局「屠边规则」）：如果所有狼人被淘汰，村民就获胜；如果狼人杀光了所有普通村民 **或** 所有神职（预言家/女巫/守卫/猎人），狼人就获胜。
在这个游戏中，我们有从1到9号共9名玩家 —— 6名村民和3名狼人。村民中有特殊角色，包括：
- 1位预言家：
    - 目标：预言家的目的是帮助村民识别狼人。
...
- 获胜条件（屠边规则）：如果所有狼人被淘汰，村民就获胜；如果狼人杀光了所有普通村民 **或** 所有神职，狼人就获胜。
...
CON.hunter_description = """- 1位猎人：
    ...
    - 能力：当猎人被狼人杀害或者在白天被放逐出局后，他可以翻开自己的身份牌并向场上任意一位活着的玩家射出一发复仇的子弹，带着这位玩家一起死亡。猎人可以选择不翻牌（即不开枪带人）。**重要例外**：当猎人被女巫毒杀时，无法发动技能，不能开枪带人（俗称"闷枪"）。
"""
