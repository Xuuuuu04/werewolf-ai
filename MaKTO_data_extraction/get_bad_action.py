...
def get_wolf_bad_action(log, role_mapping):
    """
    return: [(player_id, phase)]
    The rule the judge the unacceptable action for wolf: 
        1. wolf doesn't kill anyone.
    """
    add_to_data = []
    for i in log:
        day = i["day"]
        if i["event"] == "skill_wolf":
            if i["target"] == 0 or i["target"] is None:
                phase = f"{day}_night_skill_wolf"
                add_to_data.append((i["source"], phase))
    return add_to_data


def get_good_bad_action(log, role_mapping, game_type):
    """
    return: [(player_id, phase)]
    The rules for unacceptable action for good:
    1. witch: 1. doesn't heal anyone at day 0, 2. poison the goods
    2. guard: for 9-p game: protect anyone at day 0, for 7-p game: protect self at day 0; protect the werewolf at day >= 1
    3. hunter: kill the gods
    """
    add_to_data = []
    for i in log:
        day = i["day"]
        ...
        if event == "skill_witch":
            if day == 0 and ("heal" not in i["content"]):
                add_to_data.append((i["source"], f"{day}_night_skill_witch"))
            if day > 0 and "poison" in i["content"]:
        ...
        if event == "skill_hunter":
            if role_mapping.get(str(i["target"]), "") in ["Seer", "Witch"]:
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--self_play', action="store_true", help="whether to select from self play")
    args = argparser.parse_args()

    assert args.game_type in args.game_dir

    all_bad_actions = {} # {game_id: [(player_id, phase)]}
    villager_cnt = 0
    wolf_cnt = 0
    for sub_dir in os.listdir(args.game_dir):
        ...
        for game_id in os.listdir(sub_dir_full):
            ...
            if werewolf_is_sft:
                wolf_bad_actions = get_wolf_bad_action(content, role_mapping) # [(wolf_id, phase)]
                bad_actions_in_game_i.extend(wolf_bad_actions)
                wolf_cnt += len(wolf_bad_actions)

            # acceptable actions for good:
            if good_is_sft:
                good_bad_acions = get_good_bad_action(content, role_mapping, args.game_type) # [(good_id, phase)]
                bad_actions_in_game_i.extend(good_bad_acions)
                villager_cnt += len(good_bad_acions)

            if len(bad_actions_in_game_i) > 0:
                all_bad_actions[game_id_full] = bad_actions_in_game_i
    
    # post process
    all_bad_actions_out = {}
    cnt = 0
    for path, acc_action in all_bad_actions.items():
        ...
        for i, (player_id, phase) in enumerate(acc_action):
            if phase not in acceptable_action_by_player[player_id]:
                ...
                cnt += 1
        all_bad_actions_out[path] = acceptable_action_by_player
    print("total bad actions:", cnt, villager_cnt, wolf_cnt)

    # write to file
    output_file = os.path.join(args.out_to, f"bad_action.json")
    with open(output_file, "w") as f:
        json.dump(all_bad_actions_out, f, indent=4)
